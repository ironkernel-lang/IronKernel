...
(R-1RK), feature by feature. Of the report's 135 feature entries, **41 are verified
by a behavioural check, 14 resolve but are unchecked, and 80 are absent**; 13 belong
...
IronKernel has never implemented still appear. Known divergences — argument order of
`eval`, short-circuiting `and?`/`or?`, and the dropped `$` sigil — are listed there
too.
