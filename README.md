...
(R-1RK), feature by feature. Of the report's 135 feature entries, **99 are verified
by a behavioural check and 36 are absent**; nothing is merely bound-but-unchecked.
34 entries belong to modules the report marks optional. The matrix also reports
status per *module*, which R-1RK 1.3.2 makes the unit of conformance: **31 of 44
modules are complete**.

Every entry of chapter 12's **Numbers** (12.5), **Rational** (12.8), **Real** (12.9)
and **Complex** (12.10) modules has a passing check, and exact arithmetic meets what
R-1RK 12.3.2 requires: exact integers are of arbitrary size and promote rather than
wrapping, and dividing them gives an exact ratio, so `(/ 1 3)` is a third and three
of them are exactly one. Ratios are kept in least terms, print and read back as
`1/3`, and collapse to an integer when the denominator reduces to one. **Inexact**
(12.6) is next: keyed dynamic variables (chapter 10), which its strict-arithmetic
control is built on, are now in place, as are keyed static variables (chapter 11).
