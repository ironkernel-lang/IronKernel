...
(R-1RK), feature by feature. Of the report's 135 feature entries, **63 are verified
by a behavioural check, 12 resolve but are unchecked, and 60 are absent**; 34 belong
...
**Rational** (12.8) and **Real** (12.9) modules — the former over the existing
numeric types, since IronKernel has no exact rational representation. The remaining
numeric modules (Inexact, Narrow inexact, Complex) are not implemented.
