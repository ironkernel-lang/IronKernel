...
and **Complex** (12.10) modules has a passing check. That is weaker than claiming
those modules are *supported*: R-1RK 12.3.2 requires exact integers of arbitrary
size, and IronKernel's are 32- or 64-bit CLR values that **wrap silently** —
`(* 2147483647 2147483647)` is `1`. Arbitrary-precision exact arithmetic is the
prerequisite for supporting any of these modules, and for **Inexact** (12.6), which
additionally needs keyed dynamic variables (chapter 10) for its strict-arithmetic
control.
