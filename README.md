...
and **Complex** (12.10) modules has a passing check, and exact integers are of
arbitrary size as R-1RK 12.3.2 requires — arithmetic promotes rather than wrapping.
Full module *support* additionally needs exact ratios of arbitrary-size integers,
which are still absent. **Inexact** (12.6) needs those, plus keyed dynamic variables
(chapter 10) for its strict-arithmetic control.
