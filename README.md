...
(R-1RK), feature by feature. Of the report's 135 feature entries, **53 are verified
by a behavioural check, 12 resolve but are unchecked, and 70 are absent**; 34 belong
to modules the report marks optional.

Chapter 12's required **Numbers** module (12.5) is complete: all 14 entries are
verified, including the `div`/`mod` and `div0`/`mod0` families. The five numeric
modules the report marks optional — Inexact, Narrow inexact, Rational, Real and
Complex — are not implemented.
