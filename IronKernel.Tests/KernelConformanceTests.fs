...
let private divergences () = [
    ...
    "12.3.2", "**Exact integer arithmetic silently wraps.** The report requires every "
              + "implementation to support exact integers of arbitrary size, and exact "
              + "ratios of them when module Rational is supported. IronKernel's exact "
              + "integers are 32- or 64-bit CLR values: `(* 2147483647 2147483647)` is "
              + "`1` rather than an error or the true product, and an integer literal "
              + "wider than `Int32` needs an `L` suffix or fails to parse. Modules "
              + "Numbers and Rational therefore return wrong results for arguments "
              + "they appear to accept, which a per-entry status cannot show."
...
let private renderMatrix () =
    ...
    appendLine "12.3.2 requires exact integers of arbitrary size, which IronKernel does not"
    appendLine "have. See the divergence for 12.3.2 before reading any row as a claim of"
